...
<script>
  ...
  import { strToTime } from "@/common/utils";
  export default {
    props: {
      id: {
        type: Number,
    ...
    setup(props, ctx) {
      ...
      const flag = tmp.value ? false : true;
      ...
      const onSubmit = ruleFormRef => {
        ...
        ruleFormRef.validate(valid => {
          if (valid) {
            if (flag) {
              // 深克隆，因为dispatch操作是异步，避免清空消息后无法成功添加事件
              store.dispatch("addevent", JSON.parse(JSON.stringify(ruleForm)))
            } else {
              store.dispatch("editevent", JSON.parse(JSON.stringify(ruleForm)))
            }
            // 提交之后要清空信息
